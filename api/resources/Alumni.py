        ...
        try:
            if (filter_params['grad_year'] != ""):
                ...
                results = db.session.query(AlumniModel).join(SchoolModel).filter(AlumniModel.last_name.like(filter_params['last_name']+"%"), 
                    ...
                    AlumniModel.job_company_industry.like(filter_params['job_company_industry']+"%"),
    ...
    def get(self):
        ...
        try:
            if (filter_type == "job_company"):
                ...
                unique_options = [item.job_company for item in db.session.query(AlumniModel.job_company).distinct() ]

            elif (filter_type == "job_title"):

                unique_options = [item.job_title for item in db.session.query(AlumniModel.job_title).distinct() ]

            elif (filter_type == "job_company_industry"):

                unique_options = [item.job_company_industry for item in db.session.query(AlumniModel.job_company_industry).distinct() ]
           
            elif (filter_type == "last_name"):
                
                unique_options = [item.last_name for item in db.session.query(AlumniModel.last_name).distinct() ]
